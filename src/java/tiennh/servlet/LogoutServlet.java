...
public class LogoutServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (user != null) {
                Cookie lastLogin = new Cookie("s2020l2", "*!@*$&!$!++");
                lastLogin.setMaxAge(0);
                response.addCookie(lastLogin);
                ses.setAttribute("SES_USER", null);
            }
        } catch (Throwable t) {
            log("LogoutServlet-" + t.getMessage());
        } finally {
            ServletContext ctx = request.getServletContext();
            ResourceBundle siteMap = (ResourceBundle) ctx.getAttribute("SITE_MAP");

            RequestDispatcher rd = request.getRequestDispatcher(siteMap.getString(HOME_PAGE));
