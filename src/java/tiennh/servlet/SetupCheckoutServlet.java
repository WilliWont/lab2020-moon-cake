 ...
 * @author Will
 */
public class SetupCheckoutServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String txtAddrID = request.getParameter("txtAddrID");
        Integer addrID;
        try {
            addrID = Integer.parseInt(txtAddrID);
            if (addrID <= 0) {
                throw new Exception();
            }
        } catch (Throwable t) {
        ...
        try {
            ...
            if (user != null && addrID != null) {
                AddressTableDAO dao = new AddressTableDAO();
                AddressTableDTO addresses = dao.getAddress(addrID);

                if (addresses != null) {
                    hasAddress = true;
                    request.setAttribute("USER_ADDR", addresses);
                }
            }
        } catch (Throwable t) {
